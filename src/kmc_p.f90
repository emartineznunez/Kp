...
real(dp) :: voll,voll0,alpha_eq,G_star,G_mix,nmol,x,G,Rkj,helm
real(dp) :: rnd(2)
integer,dimension(:),allocatable :: re1,re2,pr1,pr2,n,p,p0,ni0,nui
real (dp) ,dimension(:),allocatable :: a,rate,c0,c0read,cont,mui0,xia,muistar,rate0
avog=6.0221408d23
R=0.0831447
Rkj=8.31447e-3
read(*,"(a80)") title
...
allocate(nui(2),xia(2),muistar(2),rate0(m))
n=(/ (l,l=1,nesp) /)
...
do i=1,m
   read(*,*) rate0(i),re1(i),re2(i),pr1(i),pr2(i)
   if(re1(i)/=0.and.re2(i)/=0) rate0(i)=rate0(i)/vol
   print "(t3,i4,1p,e10.2,4(i4))",i,rate0(i),re1(i),re2(i),pr1(i),pr2(i)
   if(re1(i)==re2(i)) rate0(i)=2*rate0(i)
enddo
print*, ""
...
big: do inran=1,nran
   ...
   do while(tprint<tmax)
     voll=(real(p(1),dp)+real(p(2),dp))/avog*R*temp
     do j=1,m
        if(re1(j)==0) then
! unimolecular reaction
     ...
     do while (t>=tprint) 
        ...
        print "(t3,a)","      Kc          Kp           alpha      V(L)    alpha(eq)   G/RT(mol)"
        kc=real(p(2),dp)*real(p(2),dp)/real(p(1),dp)/vol*voll0/voll
        ...
        if(alpha==0) G_mix = 0
        helm = G_star + G_mix 
        if(p(1)>0) print "(t3,(1p,e12.4),(1p,e12.4),e12.4,e12.4,e12.4,e12.4)",kc,kp,alpha,voll,alpha_eq,helm
        print*, ""
